		...
		foreach ($this->_aConditions as $iKey => $aValue)
		{
			switch ($aValue['type'])
			{
				...
				case 'select':
					...
					{
						$this->_aHtml[$iKey] = '<select '. ($aValue['type'] == 'multiselect' ? 'multiple ' :'') . (isset($aValue['id']) ? 'id="' . $aValue['id'] . '" ' : '') . 'name="search[' . $iKey . ']"' . (isset($aValue['style']) ? ' style="' . $aValue['style'] . '"' : '') . (isset($aValue['class']) ? ' class="' . $aValue['class'] . '" ' : '') . '>';
