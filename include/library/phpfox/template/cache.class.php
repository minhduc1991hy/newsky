	...
	private function _parseFunction($sFunction, $sModifiers, $sArguments)
	{
		switch ($sFunction)
		{
			...
			case 'select_gender':
				...
				$sGenders = '<select name="val[gender]" id="gender" class="form-control">' . "\n";
				...
				return $sGenders;
				break;
			case 'select_user_view_id':
				$aArgs = $this->_parseArgs($sArguments);
				if (isset($aArgs['value_title']) && strpos($aArgs['value_title'], 'phrase var=') !== false)
				{
					$aArgs['value_title'] = Phpfox::getPhrase(str_replace(array('phrase var=','"',"'"),'',$aArgs['value_title']));
				}
				$sGenders = '<select name="val[view_id]" id="view_id" class="form-control">' . "\n";
				$sGenders .= "\t\t" . '<option value="">' . (isset($aArgs['value_title']) ? $this->_removeQuote($aArgs['value_title']) : '<?php echo Phpfox::getPhrase(\'core.select\'); ?>:') . '</option>' . "\n";
				foreach (Phpfox::getService('user.custom.data')->getTextUserViewId() as $iKey => $aViewId)
				{
					$sGenders .= "\t\t\t" . '<option value="' . $iKey . '"' . $this->_parseFunction('value', '', "type='select' id='view_id' default='{$iKey}'") . '>'.$aViewId['text'].'</option>' . "\n";
				}
				$sGenders .= "\t\t" . '</select>';
				return $sGenders;
				break;
			case 'select_user_group_id':
				$aArgs = $this->_parseArgs($sArguments);
				if (isset($aArgs['value_title']) && strpos($aArgs['value_title'], 'phrase var=') !== false)
				{
					$aArgs['value_title'] = Phpfox::getPhrase(str_replace(array('phrase var=','"',"'"),'',$aArgs['value_title']));
				}
				$sGenders = '<select name="val[user_group_id]" id="user_group_id" class="form-control">' . "\n";
				$sGenders .= "\t\t" . '<option value="">' . (isset($aArgs['value_title']) ? $this->_removeQuote($aArgs['value_title']) : '<?php echo Phpfox::getPhrase(\'core.select\'); ?>:') . '</option>' . "\n";
				foreach (Phpfox::getService('user.group')->get() as $iKey => $aGroup)
				{
					$sGenders .= "\t\t\t" . '<option value="' . $aGroup['user_group_id'] . '"' . $this->_parseFunction('value', '', "type='select' id='user_group_id' default='".$aGroup['user_group_id']."'") . '>'.$aGroup['title'].'</option>' . "\n";
				}
				$sGenders .= "\t\t" . '</select>';
				return $sGenders;
				break;
			...
			/* add by Nguyen Duc */
			case 'body_class':
				return '<?php Phpfox::getBlock(\'core.template-bodyclass\'); ?>';
				break;
			case 'no_item_message':
				$aArgs = $this->_parseArgs($sArguments);
				return '<div class="no-item-message">'.((isset($aArgs['name']) ? $aArgs['name'] : 'Không có nội dung hiển thị')).'</div>';
				break;
			case 'loading':
				$sHtml = '<div class="template_loadding">';
					$sHtml .= '<div class="wrap-loadding">Đang tải dữ liệu.. <i class="fa fa-circle-o-notch fa-spin"></i></div>';
				$sHtml .= '</div>';
				return $sHtml;
			break;
			/* END by Nguyen Duc */
		...
		foreach ($aMods as $sMod)
		{
			...
			else
			{
				switch ($sMod)
				{
					...
					//add By Nguyen Duc
					case 'newsky_inventoried_unit':
						$sVariable = 'Phpfox::getService(\'manager.html\')->getNewskyInventoriedUnit(' . $sVariable . $sArg . ')';
						break;
					case 'newsky_sale_unit':
						$sVariable = 'Phpfox::getService(\'manager.html\')->getNewskySaleUnit(' . $sVariable . $sArg . ')';
						break;
					case 'newsky_position':
						$sVariable = 'Phpfox::getService(\'manager.html\')->getNewskyPosition(' . $sVariable . $sArg . ')';
						break;
					case 'newsky_unit':
						$sVariable = 'Phpfox::getService(\'manager.html\')->getNewskyUnit(' . $sVariable . $sArg . ')';
						break;
					case 'newsky_be_mat':
						$sVariable = 'Phpfox::getService(\'manager.html\')->getNewskyBeMat(' . $sVariable . $sArg . ')';
						break;
					case 'newsky_kieu_vat':
						$sVariable = 'Phpfox::getService(\'manager.html\')->getNewskyKieuVat(' . $sVariable . $sArg . ')';
						break;
					case 'newsky_khuon_duoi':
						$sVariable = 'Phpfox::getService(\'manager.html\')->getNewskyKhuonDuoi(' . $sVariable . $sArg . ')';
						break;
					case 'newsky_kieu_ranh':
						$sVariable = 'Phpfox::getService(\'manager.html\')->getNewskyKieuRanh(' . $sVariable . $sArg . ')';
						break;
					case 'newsky_TCKD':
