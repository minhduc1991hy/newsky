			...
			<div class="pager_view_more_link">
				{if !empty($aPager.nextAjaxUrl)}		
					<a href="{$aPager.nextUrl}" class="pager_view_more no_ajax_link" onclick="$.ajaxCall('{$sAjax}', 'page={$aPager.nextAjaxUrl}{$aPager.sParamsAjax}', 'GET'); return false;">
						{if !empty($aPager.icon)}{img theme=$aPager.icon class='v_middle'}{/if}
			...
			{else}
				<div class="row">
					<div class="col-sm-5">
						...
						<div class="dataTables_info text-left" role="status" aria-live="polite">{phrase var='core.page_x_of_x' current=$aPager.current total=$aPager.totalPages}</div>
						{/if}
					</div>
					<div class="col-sm-7">
							...
							<ul class="pagination">
								...
								{else}
									<li class="paginate_button first disabled">
										<a href="javascript:void(0);">{phrase var='core.first'}</a>
									</li>
								{/if}
								...
								{else}
									<li class="paginate_button previous disabled">
										<a href="javascript:void(0);">{phrase var='core.previous'}</a>
									</li>
								{/if}

								{foreach from=$aPager.urls key=sLink name=pager item=sPage}
									<li class="{if $aPager.current == $sPage}active{/if} paginate_button {if !isset($aPager.firstUrl) && $phpfox.iteration.pager == 1}first{/if}">
								...
								{else}
									<li class="paginate_button next disabled">
										<a href="javascript:void(0);">{phrase var='core.next'}</a>
									</li>
								{/if}
								
								{if isset($aPager.lastUrl)}
									<li class="paginate_button last">
										<a {if $sAjax}href="{$aPager.lastUrl}" onclick="$(this).parent().parent().parent().parent().find('.sJsPagerDisplayCount').html($.ajaxProcess('{phrase var='core.loading'}')); $.ajaxCall('{$sAjax}', 'page={$aPager.lastAjaxUrl}{$aPager.sParams}'); $Core.addUrlPager(this); return false;"{else}href="{$aPager.lastUrl}"{/if}>
											{phrase var='core.last'}
										</a>
									</li>
								{else}
									<li class="paginate_button last disabled">
										<a href="javascript:void(0);">{phrase var='core.last'}</a>
									</li>
								{/if}
							</ul>	
							<div class="clear"></div>		
