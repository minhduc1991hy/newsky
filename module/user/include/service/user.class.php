...
{	
	...
	public function isUser($mName, $bId = false, $aCoundExt = array())
	{
		(($sPlugin = Phpfox_Plugin::get('user.service_user_isuser')) ? eval($sPlugin) : false);
		
		$aCound = array();
		if($bId){
			$aCound[] = 'AND user_id = ' . (int)$mName;
		}else{
			$aCound[] = 'AND user_name = "'.$this->database()->escape($mName).'"';
		}

		if($aCoundExt){
			$aCound = array_merge($aCoundExt, $aCound);
		}
